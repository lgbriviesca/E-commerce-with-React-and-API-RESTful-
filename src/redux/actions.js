...
export const getCategoriesThunk = () => {
  ...
  export const filterTitleThunk = title => {
    return dispatch => {
        dispatch(setIsLoading(true));
        return axios.get(`https://ecommerce-api-react.herokuapp.com/api/v1/products/?query=${title}`)
             .then(res => dispatch(setProducts(res.data.data?.products))) 
            /* .then(res => dispatch(console.log(res.data.data?.products))) */
            .finally(() => dispatch(setIsLoading(false)));
    }
...
export const addPurchasesThunk = (purchase) => {
  return (dispatch) => {
    dispatch(setIsLoading(true));
    return axios
      .post( "https://ecommerce-api-react.herokuapp.com/api/v1/purchases", purchase, getConfig())
      .finally(() => dispatch(setIsLoading(false)));
  };
};
...
export const getPurchasesDoneThunk = () => {
  ...
  .get("https://ecommerce-api-react.herokuapp.com/api/v1/purchases", getConfig())
       .then(res => dispatch(setPurchasesDone(res.data.data.purchases)) && (dispatch(setCart([]))) )
