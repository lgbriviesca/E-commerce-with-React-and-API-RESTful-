           ...
           <ul >
            {purchasesDone.map((purchaseDone) => (
              <article className="purchase-card" key={purchaseDone.id} >
               ...
               <div  className="quantity" > <p>{purchaseDone.cart?.products[0]?.productsInCart.quantity}</p> </div> 
                <p className="price">{purchaseDone.cart?.products[0]?.price}</p>
                </div>
                
             {/*    {console.log(purchaseDone.cart?.products[0]?.title)} */}
              </article>
            ))}
          </ul> 
        </div>
