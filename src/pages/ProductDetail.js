...
const ProductDetail = () => {
  ...
  useEffect(() => {
    if (productFound) {
      axios
        .get(
          `https://ecommerce-api-react.herokuapp.com/api/v1/products/?category=${productFound?.category.id}`
        )
  ...
  const addProduct = () => {
    ...
    } else dispatch(addCartThunk(product));
  };
        ...
        <ul className="other-products-grid">
          {productFiltered.map(productItem => (
            <div className="other-products-card" key={productItem.id}>
              <Link
                className="link-to-other-products"
